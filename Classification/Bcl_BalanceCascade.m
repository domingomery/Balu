% ds      = Bcl_BalanceCascade(X,d,Xt,options)  Training & Testing together
...
%
%    It can adaptively  exploit the majority class examples avoiding 
...
function [ds,options] = Bcl_BalanceCascade(varargin)
...
if train
    ...
    % False positive rate that Hi should achieve
    FP = (pos_size/neg_size)^(1/(T-1));
    ...
    while t <= T
        ...
        % Remove all examples that are correctly classi?ed by Hi.
        DT = Bcl_AdaBoostM1(NEG_DATA, model);
        NEG_DATA = NEG_DATA(DT==1,:);
        neg_size = size(NEG_DATA, 1);
        
        % Incrementing loop counter
        t = t + 1;

        % Stop if the negative set has less elments that positive set
