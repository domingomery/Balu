...
function [T,p] = Bev_holdout(X,d,options)
...
if (strat)
    [XX,dd,XXt,ddt] = Bds_stratify(X,d,s);
else
    [XX,dd,XXt,ddt] = Bds_nostratify(X,d,s);
end

[dds,op] = Bcl_structure(XX,dd,XXt,b);
...
for i=1:n
    [TT,pp] = Bev_confusion(ddt,dds(:,i),nn);
    T(:,:,i) = TT;
    p(i) = pp;
    if show
