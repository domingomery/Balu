% [p,ci] = Bev_bootstrap0632(X,d,options)
%
% Toolbox: Balu
...
%       [p,ci] = Bev_bootstrap0632(X,d,op)              % Bootstrap
%
...
%       [p,ci] = Bev_bootstrap0632(X,d,op);                                   % Bootstrap 0632
%
...
function [p,ci] = Bev_bootstrap0632(X,d,options)
...
for i=1:B
    ...
    th1(i,:)            = Bev_performance(ddnb,dds,nn);
    dds                 = Bcl_structure(XXb,ddb,XXb,b);
    th2(i,:)            = Bev_performance(ddb,dds,nn);
end
...
for k=1:n
    ...
    thk = (1-k1)*th1(:,k) + k1*th2(:,k); % = 0.632*th1 + 0.368*th2;
    [i,j] = sort(thk);
    c1 = fix(B*(1-c)/2+0.5);
    c2 = B-c1;
    p(k) = mean(thk);
